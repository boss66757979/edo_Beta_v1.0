/** 书架类，管理书架 */
const app = getApp()
// var bookindexClass = function () {
//export class bookindexClass{
// module.exports = function(){
function bookindexClass(){
  /**书架类 */
  this.books = [];
  this.currentBook = {};
  // this.nowPage = {};
  this.user_path = `${wx.env.USER_DATA_PATH}/bookindex/`;
  // this.booksInfo_path = `${wx.env.USER_DATA_PATH}/bookindex/bookInfo.info`;
  this.cloud = {
    fileidPre:'cloud://release-bf6b22.7265-release-bf6b22/',
    user_bs: 'user',  // 4月12日改为 'user'  旧的：/user_bookindex
    share_bs:'share_bookindex',
  }
  this.cache = {
    key:'bookCache',
  }
  ...
  this.deleteLocalBook = function(id,info=null){
    /**删除书，根据id, info用来验证 */
    console.log('bookindexMgr.deleteBook local  ->', id)
    if(info == null){
      this.getBooksInfo()
  ...
  this.deleteCloudBook =async function (id,info=null) {
    /**删除书，根据id, info用来校验 */
    console.log('bookindexMgr.deleteBook  ->', id)
    if(info == null){
      this.getBooksInfo()
      info = this.getBookInfo(id)
    }
  ...
  this.doUpload =async function(openId,bookId,file='all'){
    ...
    if(file == 'all' || file == 'txt'){
      uploadList.push({
        cp: `user/${openId}/bookindex/${info[1]['id']}.txt`,
        fp: this.user_path + info[1]['id'] + '.txt'
      })
    }
    if(file == 'all'  || file=='info'){
      uploadList.push({
        cp: `user/${openId}/bookindex/${info[1]['id']}.info`,
        fp: this.user_path + info[1]['id'] + '.info'
      })
    }
    // var cloudPath = `user_bookindex/${openId}/${info[1]['id']}.txt`
  ...
  this.downloadBook = async function(id){
    // cloud://release-bf6b22.7265-release-bf6b22/user/oYHhN5cMc2S6WQnjzI8yAQnyYiQE/bookindex/2019412-1.42w-VQTX.info
    try{
      let openId = wx.getStorageSync('openid')
      // 云id
      let txtFile = `${this.cloud.fileidPre}user/${openId}/bookindex/${id}.txt`;
      let infoFile = `${this.cloud.fileidPre}user/${openId}/bookindex/${id}.info`;
  ...
  this.addAnnotate = function(id, annotate){
    /**annotateer={title,start,scrollY} */
    let [index,book] = this.getBookInfo(id)
    if(index != undefined && book['id'] == id){
      // 添加注释
      if (Object.prototype.toString.call(this.books[index]['annotates']) == "[object Object]"){
        if('list' in this.books[index]['annotates']){
          this.books[index]['annotates']['list'].push(annotate)
        }else{
          this.books[index]['annotates']['list'] = [annotate]
        }
      }else{
        this.books[index]['annotates'] = {}
        this.books[index]['annotates']['list'] = [annotate]
      }
      console.log('add annotate -> info',this.books[index])
      this.refreshBookInfo(index,book['id'])
    }else{
      console.log('添加注释出错，addAnnotate id=',id)
    }
  }

  this.deleteAnnotate = function(id,bi){
    let [index, book] = this.getBookInfo(id)
    if (index != undefined && book['id'] == id) {
      // 删除
      if ('list' in this.books[index]['annotates']) {
        this.books[index]['annotates']['list'].splice(bi,1)
        this.refreshBookInfo(index, book['id'])
      } else {
        // 没有注释？？
      }
    } else {
      console.log('删除注释出错，未找到book id=', id)
    }
  }
...
module.exports.bookindexClass = bookindexClass
...
// function getTempFileUrl(list) {
    ...
    // async function main() {
    //   var list = ['cloud://release-bf6b22.7265-release-bf6b22/user_bookindex/oYHhN5cMc2S6WQnjzI8yAQnyYiQE/201942-16.43w-PZBE.info']
